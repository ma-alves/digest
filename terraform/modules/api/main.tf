...
resource "aws_api_gateway_method" "subscribers_options" {
  rest_api_id   = aws_api_gateway_rest_api.api.id
  resource_id   = aws_api_gateway_resource.subscribers.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "subscribers_options" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.subscribers.id
  http_method = aws_api_gateway_method.subscribers_options.http_method
  type        = "MOCK"
...
resource "aws_api_gateway_method_response" "subscribers_options_200" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.subscribers.id
  http_method = aws_api_gateway_method.subscribers_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "subscribers_options_200" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.subscribers.id
  http_method = aws_api_gateway_method.subscribers_options.http_method
  status_code = aws_api_gateway_method_response.subscribers_options_200.status_code
...
resource "aws_api_gateway_deployment" "deploy" {
  depends_on = [
    ...
    aws_api_gateway_integration.subscribers_options,
  ]
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_method.subscribers_options.id,
    ]))
  }
